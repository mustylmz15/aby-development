  ...
  <div class="panel">
        ...
        <tbody>
          <template v-for="repair in paginatedRepairs" :key="repair.id">
            ...
            <tr 
              ...
              :class="{ 
                'bg-emerald-100 dark:bg-emerald-900/30 border-l-4 border-emerald-500': openActionId === repair.id,
            ...
            <tr v-if="openActionId === repair.id" class="bg-emerald-50 dark:bg-emerald-800 border-t border-emerald-200 dark:border-emerald-600">
              <td colspan="8" class="px-6 py-4">
                <div class="flex items-center justify-center space-x-4">
                  <button
                    @click.stop="viewRepairDetails(repair)"
                    class="btn btn-outline-primary flex items-center space-x-2"
                  >
                    <i class="fa fa-eye"></i>
                    <span>Onarım Detayı Görüntüle</span>
                  </button>
                  <button
                    @click.stop="returnToWarehouse(repair)"
    ...
    <div v-else class="text-center py-8">
      ...
      <p class="text-gray-500">
        {{ hasActiveFilters ? 'Filtrelere uygun ürün bulunamadı' : 'REPAIRED durumunda ürün bulunmuyor' }}
      </p>
    </div>
...
import ArizaDetayModal from './modals/ariza-detay-modal.vue';
...
const emit = defineEmits<{
  refresh: [];
  'view-repair-details': [repair: Repair];
  'return-to-warehouse': [repair: Repair];
}>();
...
const availableWarehouses = computed(() => {
  const warehouses = repairs.value.map(r => r.repairWarehouse.name);
...
const getReportedByName = (repair: Repair): string => {
  if (repair.reportedByUser) {
    return `${repair.reportedByUser.firstName} ${repair.reportedByUser.lastName}`;
  }
  return '-';
};

// Genişletilen satır kontrolü
const toggleActions = (repairId: string) => {
  openActionId.value = openActionId.value === repairId ? null : repairId;
};

// Buton işlemleri
const viewRepairDetails = (repair: Repair) => {
  emit('view-repair-details', repair);
};

const returnToWarehouse = (repair: Repair) => {
  emit('return-to-warehouse', repair);
};

const loadRepairs = async () => {
  try {
    repairs.value = await repairAPI.getRepairsByStatus('REPAIRED');
  } catch (error) {
    console.error('REPAIRED onarım listesi yükleme hatası:', error);
  }
};
...
const goToReturn = (repairId: string) => {
  router.push(`/onarim-merkezi/onarimdan-gonder/${repairId}`);
};
