            ...
            <div class="relative bg-white flex w-full items-center px-5 py-2.5 dark:bg-[#0e1726]">
                ...
                >
                    ...
                    <div class="flex-1 text-center hidden md:block">
                        ...
                        <div class="w-40 h-0.5 bg-gradient-to-r from-primary to-blue-600 mx-auto mt-1 mb-1"></div>
                        <p class="text-xs text-gray-500 dark:text-gray-400">
                            Envanter Takip Yönetim Sistemi
                        </p>
                    </div>
                    ...
                    <div class="dropdown shrink-0">
                        <div class="shrink-0 flex items-center justify-center">
                            <img src="/assets/images/flags/TR.svg" alt="Türkçe" class="w-5 h-5 object-cover rounded-full" />
                        </div>
                    </div>
                                ...
                                <ul class="top-11 !py-0 text-dark dark:text-white-dark w-[300px] sm:w-[375px] text-xs">
                                    <li class="mb-5">
                                        <div class="overflow-hidden relative rounded-t-md !p-5 text-white">
                                            ...
                                            ></div>
                                            <h4 class="font-semibold relative z-10 text-lg">Sistem Mesajları</h4>
                                        </div>
                                    </li>
                                    ...
                                    <template v-if="!messages.length">
                                        <li class="mb-5">
                                            <div class="!grid place-content-center hover:!bg-transparent text-lg min-h-[200px]">
                                               ...
                                               Tüm Sistem Mesajlarını Okudunuz!
                                            </div>
                                        </li>
                                    </template>
                                </ul>
                    ...
                    <div class="dropdown shrink-0">
                        <Popper :placement="store.rtlClass === 'rtl' ? 'bottom-end' : 'bottom-start'" offsetDistance="8">
                            ...
                            >
                                ...
                                <!-- Onay bildirimleri badge'i -->
                                <span v-if="approvalNotificationCount > 0" class="flex absolute w-3 h-3 ltr:right-0 rtl:left-0 top-0">
                                    <span
                                        class="animate-ping absolute ltr:-left-[3px] rtl:-right-[3px] -top-[3px] inline-flex h-full w-full rounded-full opacity-75"
                                        :class="{
                                            'bg-danger/50': hasUrgentApprovals,
                                            'bg-success/50': !hasUrgentApprovals
                                        }"
                                    ></span>
                                    <span 
                                        class="relative inline-flex rounded-full w-[6px] h-[6px]"
                                        :class="{
                                            'bg-danger': hasUrgentApprovals,
                                            'bg-success': !hasUrgentApprovals
                                        }"
                                    ></span>
                                </span>
                                
                                <!-- Normal bildirimler badge'i -->
                                <span v-else-if="notifications.length > 0" class="flex absolute w-3 h-3 ltr:right-0 rtl:left-0 top-0">
                                    <span
                                ...
                                <ul class="!py-0 text-dark dark:text-white-dark w-[300px] sm:w-[350px] divide-y dark:divide-white/10">
                                    <li>
                                        <div class="flex items-center px-4 py-2 justify-between font-semibold">
                                            <h4 class="text-lg">Bildirimler</h4>
                                            <template v-if="totalNotificationCount > 0">
                                                <span class="badge bg-primary/80">{{ totalNotificationCount }} Yeni</span>
                                            </template>
                                        </div>
                                    </li>
                                    
                                    <!-- Onay Bildirimleri Bölümü -->
                                    <template v-if="approvalNotifications.length > 0">
                                        <li>
                                            <div class="px-4 py-2 bg-primary/5 dark:bg-primary/10">
                                                <div class="flex items-center justify-between">
                                                    ...
                                                    <button 
                                                        @click="navigateToApprovalCenter(), close()"
                                        ...
                                        <template v-for="notification in approvalNotifications" :key="notification.id">
                                            <li class="dark:text-white-light/90">
                                                <div 
                                                    class="group flex items-center px-4 py-3 cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-800"
                                                    @click="navigateToApprovalCenter(), close()"
                                                >
                                                    <div class="grid place-content-center rounded">
                                                        <div class="w-10 h-10 rounded-full flex items-center justify-center text-lg"
                                                             :class="{
                                                                'bg-red-100 text-red-600 dark:bg-red-900/30': notification.urgency === 'critical',
                                                                'bg-orange-100 text-orange-600 dark:bg-orange-900/30': notification.urgency === 'urgent',
                                                                'bg-blue-100 text-blue-600 dark:bg-blue-900/30': notification.urgency === 'normal'
                                                             }">
                                                            {{ notification.icon }}
                                                        </div>
                                                    </div>
                                                    <div class="ltr:pl-3 rtl:pr-3 flex-1">
                                                        <h6 class="text-sm font-medium" :class="{
                                                            'text-red-600 dark:text-red-400': notification.urgency === 'critical',
                                                            'text-orange-600 dark:text-orange-400': notification.urgency === 'urgent'
                                                        }">
                                    ...
                                    <!-- Normal Bildirimler kaldırıldı - sadece onay bildirimleri gösterilecek -->
                                        ...
                                        <li>
                                            <div class="p-4">
                                                <button 
                                                    ...
                                                    @click="navigateToApprovalCenter(), close()"
                                                >
                                                    Tüm Onayları Görüntüle ({{ approvalNotificationCount }})
                                                </button>
                                            </div>
                                        </li>
                                    </template>
                    ...
                    <div class="dropdown shrink-0">
                        <Popper :placement="store.rtlClass === 'rtl' ? 'bottom-end' : 'bottom-start'" offsetDistance="8">
                            ...
                            <template #content="{ close }">
                                <ul class="text-dark dark:text-white-dark !py-0 w-[350px] font-semibold dark:text-white-light/90">
                                    <li>
                                        <div class="flex items-center px-4 py-4">
                                            <div class="flex-none">
                                                <img class="rounded-md w-10 h-10 object-cover" :src="user.avatar || '/assets/images/user-profile.jpeg'" alt="" />
                                            </div>
                                            <div class="ltr:pl-4 rtl:pr-4 truncate">
                                                <h4 class="text-base">
                                                    {{ user.firstName || '-' }} {{ user.lastName || '' }}
                                                    <span v-if="user.isActive" class="text-xs bg-success-light rounded text-success px-1 ltr:ml-2 rtl:ml-2">Aktif</span>
                                                </h4>
...
<script lang="ts" setup>
    ...
    // Onay bildirimleri için composable
    import { useGlobalApprovalNotifications } from '@/views/approval/composables/useApprovalNotifications';
    ...
    // Onay bildirimleri için gerçek API composable
    const { 
        notifications: approvalNotificationsData,
        totalNotifications,
        hasNotifications,
    ...
    const approvalNotifications = computed(() => {
        return approvalNotificationsData.value.map(notification => ({
            id: notification.id,
            title: notification.title,
            message: notification.message,
            urgency: notification.priority === 'high' ? 'critical' : notification.priority === 'medium' ? 'urgent' : 'normal',
    ...
    const approvalNotificationCount = computed(() => {
        return totalNotifications.value;
    });
    
    const hasUrgentApprovals = computed(() => {
        return highPriorityNotifications.value.length > 0;
    });
    ...
    const handleLogout = async () => {
        try {
            ...
            // User objesini temizle
            Object.keys(user).forEach(k => delete user[k]);
            
            // ABY Portal'a yönlendir
            const abyPortalUrl = import.meta.env.VITE_ABY_PORTAL_URL || 'http://localhost:5173';
            window.location.href = abyPortalUrl;
        } catch (error) {
            console.error('Logout error:', error);
            // Hata olsa bile ABY Portal'a yönlendir
            const abyPortalUrl = import.meta.env.VITE_ABY_PORTAL_URL || 'http://localhost:5173';
            window.location.href = abyPortalUrl;
        }
    };

    // Onay merkezi navigation fonksiyonları
    const navigateToApprovalCenter = () => {
        router.push('/approval');
    };

    // Toplam bildirim sayısı - sadece onay bildirimleri
    const totalNotificationCount = computed(() => {
        return approvalNotificationCount.value;
    });
    ...
    const notifications = ref<any[]>([
        // Mock datalar temizlendi - sadece onay bildirimleri gösterilecek
    ]);
    ...
    const messages = ref([
        {
            ...
            title: 'Sistem Mesajları Burada Gösterilecek!',
            message: 'Yönetici bildirimleri, sistem mesajları ve diğer önemli duyurular burada görüntülenecek.',
    ...
    // removeNotification fonksiyonu kaldırıldı - artık sadece onay bildirimleri var
